...
**This branch: `text-booking`.** The operator screen goes in as plain text tokens in the model's vision slot (after
`<unit>`, before the audio of each one-second chunk): no frame is rendered, no image is encoded, and the behaviour is
the same as on the main branch [`vision-booking`](../../tree/vision-booking), where the screen is a rendered frame. The
two branches differ only in that channel; this one is the evidence that the channel matters, not the pixels.
...
- **Audio is only the customer.** No synthetic voice, no injected audio. The model's natural trigger to speak is
  acoustic.
- **The vision slot carries the operator screen** (as text on this branch). Two or three lines, all of them sentences the operator can say to the customer
...
model reads a new screen 1.5-3 s after the customer's line. On this branch the screen is at most 160 text tokens per
chunk instead of a 448×448 frame; the numbers above are from the frame channel, and `tools/hud_run_compare.py` runs the
A/B on the recorded registries.
...
| state machine replay (recorded scenarios + regression cases) | `python tools/fsm_replay_test.py` | 72/72 |
...
| `MiniCPMO45/` | upstream model code with our changes: `screen_text` in the vision slot of the unit (this branch), `force_speak` override, per-session KV sliding window (`set_sliding_window`, `window_stats`), text injection hooks, decoder fixes (temperature applied to chunk end, repetition-penalty sign, `<|listen|>` token id, word-aligned chunk cuts) |
| `core/`, `py_backend/`, `static/duplex/` | plumbing of the new session fields (`screen_text`, `force_speak`, `sliding_window_*`) |
| `tools/*_eval*.py`, `fsm_replay_test.py`, `sigma_probe.py`, `test_norm.py`, `hud_run_compare.py` | benches above, and the frame-vs-text A/B on recorded runs |
