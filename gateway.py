...
_ALL_DAY = {"", "all day", "allday", "all-day", "whole day", "tutto il giorno", "giornata", "day", "any", "anytime", "any time"}
...
def _hud_free_hours(month: str, day: str):
    """Ore libere (9-18) di un giorno, dal gestionale: stato da mostrare quando manca l'ora o l'ora e' occupata."""
    if month not in _MONTHS or not str(day).isdigit():
        return []
...
def _hud_supervise_code(fsm, omni_text: str, heard_args: dict):
    ...
    for k in ("month", "day", "time"):
        ...
        if v and sl.get(k) and not tent.get(k):
            ...
            if norm != cur:
                # (14/09) niente etichette: ogni riga dello schermo dev'essere una frase pronunciabile con il valore SOLIDO del cliente
                return "yellow", f"Sorry, I mean {_hud_when(sl) or 'the date you said'}."
    return "green", ""
